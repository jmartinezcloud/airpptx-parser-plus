...
export default class ParagraphParser {
    ...
    public static getParagraph(paragraph, style): Paragraph {
        ...
        let contents = textElements.map((txtElement) => {
            const content: Content = {
                text: txtElement["a:t"] || "",
                textCharacterProperties: this.determineTextProperties(getValueAtPath(txtElement, '["a:rPr"][0]'), paragraph, style),
            };
    ...
    public static extractParagraphElements(paragraphInfo: any[], isListContent): PowerpointElement["paragraph"] {
        const paragraphs = getValueAtPath(paragraphInfo, '["a:p"]');
        const paragraphStyle = getValueAtPath(paragraphInfo, '["a:lstStyle"][0]');
    ...
    /**a:rPr */
    public static determineTextProperties(textProperties, paragraph, style): Content["textCharacterProperties"] {
        const size = getValueAtPath(style, '["a:lvl1pPr"][0]["a:defRPr"][0]["$"].sz') || getValueAtPath(paragraph, '["a:pPr"][0]["a:defRPr"][0]["$"].sz') || 1200;
        const font = getValueAtPath(style, '["a:lvl1pPr"][0]["a:defRPr"][0]["a:latin"][0]["$"].typeface') || getValueAtPath(paragraph, '["a:pPr"][0]["a:defRPr"][0]["a:latin"][0]["$"].typeface') || "Helvetica";
        const fillColor = getValueAtPath(style, '["a:lvl1pPr"][0]["a:defRPr"][0]["a:solidFill"][0]["a:srgbClr"][0]["$"].val') || getValueAtPath(paragraph, '["a:pPr"][0]["a:defRPr"][0]["a:solidFill"][0]["a:srgbClr"][0]["$"].val') || "000000";
        ...
        return {
            size: parseInt(getValueAtPath(textProperties, '["$"].sz') || defaultProperties.size),
